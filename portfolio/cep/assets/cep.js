...
$('#fullpage').fullpage({
				anchors: ['cep', 'ingreso', 'motivacion', 'otros'],
				sectionsColor: ['#6baf77', '#3b7144', '#323539'],
				scrollOverflow: true
			});
	...
	var $latitude = -31.6092669,
		$longitude = -60.6970409,
		$map_zoom = 14;
    ...
    function quotesadd() {
      ...
      var arraydata = {
       ...
       $.ajax({ 
           url: 'http://microtells.us:3333/addquote',
           type: 'POST',
            dataType: "json",
              data:  JSON.stringify({ "quoteData": arraydata}),
...
$(function() {
  ...
  // esconder el atomo
  $('.backgroundbeforeallisloaded').slideUp("slow", function() { $('.backgroundbeforeallisloaded').remove();});
...
function submit(e) {
	$('.cep-contact').addClass('submitted');
	e.preventDefault();
	setTimeout(
  function() 
  {
    $('.thanks-text').fadeIn('slow');
    $('.envelope').css('height','0px');
  }, 2000);
	
}

function reset() {
	$('.thanks-text').fadeOut('fast');
	setTimeout( function() 
  {
   $('.cep-contact').removeClass('submitted');
	$('.sign-up-form')[0].reset();
	$('.envelope').css('height','250px');
  }, 1000);
